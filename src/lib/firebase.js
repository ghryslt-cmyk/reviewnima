...
// Anime episode management functions
...
export const updateAnimeEpisode = async (animeId, episodeId, episodeData) => {
  try {
    const docRef = doc(db, 'anime', animeId, 'episodes', episodeId);
    await updateDoc(docRef, {
      ...episodeData,
      updatedAt: serverTimestamp()
    });
  } catch (error) {
    console.error('Error updating anime episode:', error);
...
export const deleteAnimeEpisode = async (animeId, episodeId) => {
  try {
    ...
    console.error('Error deleting anime episode:', error);
